...
(1, 2, NOW() - INTERVAL 5 DAY, 'Completed', 159.98),
(2, 2, NOW() - INTERVAL 4 DAY, 'Processing', 89.99),
(3, 4, NOW() - INTERVAL 3 DAY, 'Completed', 69.99),
(4, 5, NOW() - INTERVAL 2 DAY, 'Cancelled', 74.99),
...
(1, 1, 2, 2, 79.99, 159.98, NOW() - INTERVAL 5 DAY, 'Credit Card'),
(7, 2, 2, 1, 89.99, 89.99, NOW() - INTERVAL 4 DAY, 'PayPal'),
(4, 3, 4, 1, 69.99, 69.99, NOW() - INTERVAL 3 DAY, 'Debit Card'),
(10, 4, 5, 1, 74.99, 74.99, NOW() - INTERVAL 2 DAY, 'Credit Card'),
(13, 5, 2, 1, 149.99, 149.99, NOW() - INTERVAL 1 DAY, 'Debit Card');

-- =========================
-- 120 ORDERS FOR MYSQL
-- =========================
...
SELECT
    c.customer_id,
    CASE
        WHEN MOD(s.i, 3) = 1 THEN su.manager_id
        WHEN MOD(s.i, 3) = 2 THEN su.staff_id
        ELSE su.analyst_id
    END AS user_id,
    NOW() - INTERVAL (121 - s.i) DAY AS order_date,
    CASE
        WHEN MOD(s.i, 10) IN (1,2,3,4,5,6) THEN 'Completed'
        WHEN MOD(s.i, 10) IN (7,8) THEN 'Processing'
        ELSE 'Cancelled'
    END AS status,
    ROUND(45 + MOD(s.i, 11) * 9 + MOD(s.i, 4) * 4.75, 2) AS total_amount
FROM seq s
...
SELECT
    ...
    CASE
        ...
        ELSE su.analyst_id
    END AS user_id,
    MOD(s.i - 1, 3) + 1 AS quantity_sold,
    p.unit_price,
    ROUND(p.unit_price * (MOD(s.i - 1, 3) + 1), 2) AS total_amount,
    NOW() - INTERVAL (121 - s.i) DAY AS sale_date,
    CASE
        WHEN MOD(s.i, 4) = 1 THEN 'Credit Card'
        WHEN MOD(s.i, 4) = 2 THEN 'Debit Card'
        WHEN MOD(s.i, 4) = 3 THEN 'PayPal'
        ELSE 'Cash'
